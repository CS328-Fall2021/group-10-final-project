...
class FeatureExtractor():
    ...
    def _compute_formant_features(self, window):
        ...
        bins, bin_edges = np.histogram(frequencies, bins=10)
        
        #print(bins)
        return bins # returns dummy value; replace this with the features you extract

    def _compute_fft(self, window):
        return [np.fft.rfft(window).real.astype(float)[0]]
    ...
    def _compute_delta_coefficients(self, window, n=2):
        ...
        denom = 2 * np.sum(np.arange(1, n+1) ** 2)
        for i in range(2, n_frames-2):
            
            num = 0
            for j in range(1, n+1):
                num += i * (np.subtract(mfccs[i+j],mfccs[i-j]))
                
            delta_coefs.append(np.divide(num, denom))
            
        return np.array(delta_coefs).flatten() # returns dummy value; replace this with the features you extract
    ...
    def extract_features(self, window, debug=True):
        ...
        x = np.append(x, self._compute_fft(window))
